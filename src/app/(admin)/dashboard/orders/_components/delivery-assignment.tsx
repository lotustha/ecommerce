...
const LOGISTICS_PARTNERS = [
  ...
  },
  {
    ...
    name: "Nepal Can Move",
    icon: Truck,
    desc: "Nationwide delivery and logistics API.",
  },
];

// ==========================================
// SUB-COMPONENTS (UI Extraction)
// ==========================================

function AssignedDeliveryView({
  order,
  handlePrintLabel,
  setCancelModalOpen,
}: any) {
  return (
    <div className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
      {order.trackingCode && (
        <div className="p-5 bg-success/10 border border-success/20 rounded-2xl space-y-4 shadow-sm relative overflow-hidden">
          ...
          <div className="bg-base-100 p-4 rounded-xl border border-base-200 shadow-sm">
            <p className="text-xs opacity-60 mb-1 font-bold uppercase tracking-wider">
              Consignment ID
            </p>
            <div className="flex items-center justify-between">
              <p className="font-mono font-black text-xl text-base-content">
                {order.trackingCode}
              </p>
              <button
          ...
          <div className="flex gap-2">
            <a
              href={
                order.courier === "NCM"
                  ? `https://nepalcanmove.com/tracking?id=${order.trackingCode}`
                  : `https://parcel.pathao.com/public-tracking?consignment_id=${order.trackingCode}`
              }
              target="_blank"
              rel="noreferrer"
        ...
        <div className="p-5 bg-base-100 rounded-2xl space-y-4 border-2 border-primary/20 shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-xs opacity-60 font-bold uppercase tracking-widest truncate">
                Currently Assigned
              </p>
              <p className="font-black text-lg truncate text-base-content">
                {order.rider.name}
              </p>
            </div>
          </div>
          <button
            onClick={handlePrintLabel}
            className="btn btn-outline border-base-300 hover:border-primary hover:bg-primary hover:text-white btn-block shadow-sm"
          >
      ...
      <button
        ...
        className="btn btn-outline btn-error btn-sm w-full group"
      >
        <RefreshCcw
          size={14}
...
function PartnerSelector({
  ...
  const filteredPartners = LOGISTICS_PARTNERS.filter(
    (p) =>
  ...
  return (
    <div className="mb-6 relative" ref={partnerDropdownRef}>
      <label className="label text-xs font-bold uppercase opacity-60 pb-1 px-0">
        Logistics Partner
      </label>
      <div
      ...
      >
        ...
        <ChevronDown
          size={16}
          className={`opacity-50 transition-transform duration-200 ${partnerDropdownOpen ? "rotate-180" : ""}`}
        />
      </div>
          ...
          <div className="max-h-48 overflow-y-auto space-y-1 scrollbar-thin">
            {filteredPartners.map((partner) => {
              ...
              return (
                ...
                >
                  <div
                    className={`p-2 rounded-lg ${isSelected ? "bg-primary/20" : "bg-base-100 shadow-sm border border-base-200"}`}
                  >
                    <Icon
                      size={16}
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    <span className="text-[10px] font-medium opacity-60 leading-tight mt-0.5">
                      {partner.desc}
                    </span>
                  </div>
                  {isSelected && (
        ...
        <div className="form-control mb-5">
          ...
          <div className="flex gap-2">
            <select
              ...
              disabled={isPending || orderStatus === "CANCELLED"}
            >
            ...
            <button
              ...
              disabled={
                isPending || !selectedRider || orderStatus === "CANCELLED"
              }
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex-1 min-w-[200px]">
              ...
              <p className="text-xs opacity-60 leading-tight">
                Adjust the shipping cost for your internal rider.
              </p>
            </div>
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <div className="relative flex-1 sm:w-32">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold opacity-50">
                  Rs.
                </span>
                <input
                  type="number"
              ...
              {Number(internalShippingCost) !== Number(orderShippingCost) ? (
                <button
                  onClick={handleSyncInternalShipping}
                  disabled={isPending || internalShippingCost === ""}
                  className="btn btn-sm btn-neutral text-white shrink-0"
                >
                  Update Total
                </button>
              ) : (
          ...
          {shippingCost !== null && (
            <div className="flex justify-between items-center text-sm p-3 bg-base-100 rounded-lg border border-base-300 mt-3">
              <div className="flex flex-col min-w-0 pr-2">
                <span className="opacity-60 font-bold uppercase text-[10px] truncate">
                  Pathao Delivery Fee
                </span>
                {shippingCost !== Number(orderShippingCost) ? (
                  <button
                    type="button"
...
// ✅ UPDATED: NCM Form now receives dynamic loading state and refresh action
function NcmForm({
  shippingAddress,
  ncmBranches,
  selectedNcmBranch,
  setSelectedNcmBranch,
  weight,
  setWeight,
  shippingCost,
  orderShippingCost,
    ...
    <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            <div className="form-control col-span-2">
              <div className="flex justify-between items-center py-1">
                <label className="label text-xs font-bold p-0">
                  Destination Branch
                </label>
                <button
                  type="button"
              ...
              <select
                className="select select-bordered select-sm w-full"
                value={selectedNcmBranch}
                onChange={(e) => setSelectedNcmBranch(e.target.value)}
                disabled={isLoadingNcm}
              >
                <option value="">
                ...
                {ncmBranches.map((b: any, idx: number) => {
                  // Fallback logic to catch different JSON payload structures
                  const val =
      ...
      {!selectedNcmBranch && (
        <div className="alert alert-warning bg-warning/10 text-warning-content text-xs rounded-xl p-3 border-warning/20 flex gap-2">
          <AlertTriangle size={16} className="shrink-0" />
          <span>
            Please ensure a Destination Branch is selected before dispatching.
          </span>
        </div>
      )}
      <button
        onClick={handleOpenModal}
        disabled={
          isPending || !selectedNcmBranch || orderStatus === "CANCELLED"
        }
        className="btn btn-error btn-block text-white shadow-lg h-12"
      >
        {isPending ? (
    ...
    <dialog ref={modalRef} className="modal modal-bottom sm:modal-middle">
      <div className="modal-box">
        ...
        <div className="alert alert-info bg-info/10 text-info-content text-xs rounded-xl p-3 border-none mb-6">
          ...
          <span>
            Please review the delivery details carefully. This will immediately
            create a consignment in{" "}
            {deliveryMode === "PATHAO" ? "Pathao's" : "the"} system.
          </span>
        </div>
        ...
        <div className="p-6 flex flex-col items-center text-center">
          ...
          <p className="text-base-content/60 text-sm mb-6 leading-relaxed">
            ...
            {hasTrackingCode && (
              <span className="block mt-2 font-medium text-warning">
                Note: If the external courier has already picked up the package,
                you must also contact support directly to halt the delivery.
              </span>
            )}
          </p>
          <div className="flex gap-3 w-full">
            ...
            <button
              ...
              className="btn btn-error flex-1 text-white shadow-lg shadow-error/30"
            >
              {isCancelling ? (
                <span className="loading loading-spinner"></span>
              ) : (
...
export default function DeliveryAssignment({
  ...
  const initialNcmBranch = shippingAddress.logistics?.ncmBranch;
  const savedCourierId = order.courier?.toUpperCase();
  const isPathaoDelivery = !!initialCityId || savedCourierId === "PATHAO";
  const isNcmDelivery = !!initialNcmBranch || savedCourierId === "NCM";

  const [deliveryMode, setDeliveryMode] = useState<string>(
    isPathaoDelivery ? "PATHAO" : isNcmDelivery ? "NCM" : "STORE",
  );
  const [partnerDropdownOpen, setPartnerDropdownOpen] = useState(false);
  const [partnerSearch, setPartnerSearch] = useState("");
  const partnerDropdownRef = useRef<HTMLDivElement>(null);

  const [internalShippingCost, setInternalShippingCost] = useState<number | "">(
  ...
  const [selectedNcmBranch, setSelectedNcmBranch] = useState<string>(
    initialNcmBranch || "",
  );
  ...
  // --- NEW: ROBUST NCM LOADER ---
  const loadNcmBranches = async () => {
    setIsLoadingNcm(true);
    try {
      const data = await fetchNcmBranches();
      let branches = [];
      ...
      if (!branches || branches.length === 0) {
        toast.error(
          "NCM list is empty. Please check your API key in Settings.",
          { id: "ncm-empty" },
        );
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (!isAssigned) {
      fetchCities()
        .then((data) => {
          if (data && Array.isArray(data)) setCities(data);
        })
        .catch(console.error);
    }
  ...
  useEffect(() => {
    if (!isAssigned && deliveryMode === "NCM" && ncmBranches.length === 0) {
      loadNcmBranches();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAssigned, deliveryMode]);
  ...
  useEffect(() => {
    if (
      ...
      calculateShipping({
        provider: "NCM",
        ncm_destination: selectedNcmBranch,
        items: JSON.parse(orderItemsString),
        override_weight:
          debouncedWeight !== "" ? Number(debouncedWeight) : undefined,
      })
  ...
  const handleSyncShipping = () => {
    ...
    startTransition(async () => {
      ...
      else toast.success("Order total updated!");
    });
  };
  ...
  const handleSyncInternalShipping = () => {
    ...
    startTransition(async () => {
      const res = await updateOrderShippingCost(
        order.id,
        Number(internalShippingCost),
      );
      if (res.error) toast.error(res.error);
      else toast.success("Internal delivery fee synced!");
    });
  };
  ...
  const confirmPushToPathao = () => {
    startTransition(async () => {
      const res = await assignDelivery(order.id, {
        ...
        amount_to_collect: Math.round(defaultCodAmount),
    ...
    startTransition(async () => {
      const res = await assignDelivery(order.id, {
        ...
        recipient_name:
          shippingAddress.fullName || order.user?.name || "Customer",
        recipient_phone: shippingAddress.phone || order.phone || "",
        recipient_address: `${shippingAddress.street}, ${shippingAddress.city}, ${shippingAddress.district}`,
        recipient_branch: selectedNcmBranch,
        item_weight: weight !== "" ? Number(weight) : 1,
        item_description: `Order #${order.id.slice(-6).toUpperCase()}`,
      });
      if (res.success) {
        toast.success("Order pushed to NCM successfully!");
        modalRef.current?.close();
      } else {
        toast.error(res.error || "Failed to dispatch to NCM");
        modalRef.current?.close();
      }
    });
  };
  ...
  const handlePrintLabel = (e: React.MouseEvent) => {
    ...
    if (printWindow) {
      ...
      const htmlContent = `
        <!DOCTYPE html><html><head><title>Print Label - Order #${order.id.slice(-6).toUpperCase()}</title><style>
          @page { size: 4in 6in; margin: 0; } body { font-family: Arial, sans-serif; padding: 12px; margin: 0; color: #000; box-sizing: border-box; }
          .header { text-align: center; border-bottom: 2px solid #000; padding-bottom: 8px; margin-bottom: 12px; } .header h1 { font-size: 20px; text-transform: uppercase; margin: 0; }
          .cod-box { border: 4px solid #000; padding: 6px; margin-bottom: 12px; text-align: center; border-radius: 6px; } .cod-box h2 { font-size: 28px; margin: 0; }
          .addresses { display: flex; justify-content: space-between; margin-bottom: 12px; border-bottom: 2px solid #000; padding-bottom: 8px; gap: 8px; } .address-block { width: 48%; }
          ...
          .items { font-size: 11px; font-weight: bold; } .item-row { display: flex; justify-content: space-between; border-bottom: 1px dashed #ccc; padding-bottom: 4px; margin-bottom: 4px; }
          .footer { margin-top: 12px; text-align: center; font-size: 9px; } @media print { .no-print { display: none; } }
        </style></head><body>
          <div class="header"><h1>${settings?.storeName || "Nepal E-com"}</h1><p>${settings?.storeAddress || "Online Store"} | 📞 ${settings?.storePhone || "Contact"}</p></div>
          <div class="cod-box"><p style="font-size:10px;margin:0;">CASH TO COLLECT</p><h2>${defaultCodAmount > 0 ? `Rs. ${defaultCodAmount.toLocaleString()}` : "PRE-PAID"}</h2></div>
          <div class="addresses">
            <div class="address-block"><span style="font-size:9px;font-weight:bold;">SENDER:</span><h3 style="margin:2px 0;">${settings?.storeName || "Store"}</h3><p style="margin:0;font-size:11px;">${settings?.storeAddress || "N/A"}</p></div>
            <div class="address-block" style="text-align: right;"><span style="font-size:9px;font-weight:bold;">DELIVER TO:</span><h3 style="margin:2px 0;">${shippingAddress.fullName || order.user?.name || "Customer"}</h3>
            <p style="margin:0;font-size:11px;">${shippingAddress.street || "N/A"}<br/>${selectedCityName !== "N/A" ? selectedCityName : shippingAddress.city || "N/A"}, ${selectedZoneName !== "N/A" ? selectedZoneName : shippingAddress.district || "N/A"}</p><p style="margin:4px 0 0 0;font-size:11px;font-weight:bold;">📞 ${shippingAddress.phone || order.phone || "N/A"}</p></div>
          </div>
          <div class="info-grid"><div><p style="margin:0;">Order ID:</p><p style="font-family:monospace;font-weight:bold;margin:0;">#${order.id.slice(-8).toUpperCase()}</p></div>
          <div style="text-align:right;"><p style="margin:0;">Courier:</p><p style="font-weight:bold;margin:0;">${order.courier || order.rider?.name || "Store Courier"}</p></div></div>
          ${order.trackingCode ? `<div style="border-bottom: 2px solid #000; padding-bottom: 8px; margin-bottom: 12px; font-size: 11px;"><p style="margin:0;">Tracking ID:</p><p style="font-family:monospace;font-size:16px;font-weight:bold;margin:0;">${order.trackingCode}</p></div>` : ""}
          <div class="items"><div style="margin-bottom:4px;border-bottom:1px solid #ccc;">Contents (${order.items?.length || 0})</div>
          ${order.items?.map((item: any) => `<div class="item-row"><span>${item.name || item.product?.name || "Item"}</span><span>x${item.quantity}</span></div>`).join("") || ""}</div>
          <div class="no-print" style="margin-top: 20px; text-align: center;"><button onclick="window.print()">Print Label</button></div>
    ...
    <div className="card bg-base-100 shadow-sm border border-base-200">
      <div className="card-body p-6">
        <h2 className="card-title text-lg flex items-center gap-2 mb-4 border-b pb-3 border-base-200">
          <Truck size={20} className="text-primary" /> Logistics & Assignment
        </h2>
        ...
        {isAssigned ? (
          ...
          <>
            <PartnerSelector
              ...
              setPartnerDropdownOpen={setPartnerDropdownOpen}
            ...
            {deliveryMode === "STORE" && (
              <StoreForm
                ...
                internalShippingCost={internalShippingCost}
                setInternalShippingCost={setInternalShippingCost}
                orderShippingCost={order.shippingCost}
                handleSyncInternalShipping={handleSyncInternalShipping}
            ...
            {deliveryMode === "PATHAO" && (
              <PathaoForm
                ...
                handleSyncShipping={handleSyncShipping}
                handleOpenModal={() => modalRef.current?.showModal()}
                isPending={isPending}
                orderStatus={order.status}
              />
            )}
            {/* ✅ UPDATED NCM FORM PROPS */}
            {deliveryMode === "NCM" && (
              <NcmForm
                shippingAddress={shippingAddress}
                ncmBranches={ncmBranches}
                selectedNcmBranch={selectedNcmBranch}
                setSelectedNcmBranch={setSelectedNcmBranch}
                weight={weight}
                setWeight={setWeight}
                shippingCost={shippingCost}
                orderShippingCost={order.shippingCost}
                ...
                isPending={isPending}
                orderStatus={order.status}
                isLoadingNcm={isLoadingNcm}
                loadNcmBranches={loadNcmBranches}
              />
            )}
          </>
        )}
        ...
        <ConfirmDispatchModal
          ...
          weight={weight}
          shippingCost={shippingCost}
          defaultCodAmount={defaultCodAmount}
          isPending={isPending}
          confirmAction={
            deliveryMode === "PATHAO"
              ? confirmPushToPathao
          ...
          cancelAction={() => modalRef.current?.close()}
        />
        ...
        <CancelAssignmentModal
          ...
          hasTrackingCode={!!order.trackingCode}
        />
      </div>
    </div>
  );
