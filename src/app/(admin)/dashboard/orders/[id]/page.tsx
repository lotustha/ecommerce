                  ...
                  <tbody>
                    {order.items.map((item) => {
                      ...
                      return (
                        <tr
                          key={item.id}
                          className="border-b border-base-200/50 last:border-0 hover:bg-base-200/20 transition-colors"
                        >
                          <td>
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                {item.variantId && (
                                  <p className="text-xs opacity-60 mt-0.5">
                                    Variant ID: {item.variantId.slice(-6)}
                                  </p>
                                )}
                              </div>
                            </div>
                ...
                {discountAmount > 0 && (
                  <div className="flex justify-between items-center text-sm bg-success/10 text-success p-2.5 rounded-xl border border-success/20 mt-2">
                    <span className="font-bold flex items-center gap-1.5">
                      ...
                      {couponCode && (
                        <span className="badge badge-success text-white badge-xs py-2 uppercase tracking-wider">
                          {couponCode}
                        </span>
                      )}
                    </span>
        ...
        <div className="space-y-6">
              ...
              <div className="bg-base-200/50 p-4 rounded-xl space-y-1.5 text-sm">
                ...
                <div className="mt-4 pt-4 border-t border-base-300">
                  ...
                  <p className="font-mono text-xs">
                    City:{" "}
                    {(order as any).pathaoCityId ||
                      shippingAddress.logistics?.pathaoCityId ||
                      "N/A"}
                  </p>
                  <p className="font-mono text-xs">
                    Zone:{" "}
                    {(order as any).pathaoZoneId ||
                      shippingAddress.logistics?.pathaoZoneId ||
                      "N/A"}
                  </p>
                  <p className="font-mono text-xs">
                    Area:{" "}
                    {(order as any).pathaoAreaId ||
                      shippingAddress.logistics?.pathaoAreaId ||
                      "N/A"}
                  </p>
                </div>
              </div>
          ...
          <div className="card bg-base-100 shadow-sm border border-base-200">
            <div className="card-body p-6">
              ...
              <div className="space-y-4">
                <div>
                  <p className="text-[10px] uppercase font-bold opacity-50 tracking-wider mb-1">
                    Method
                  </p>
                ...
                <div>
                  <p className="text-[10px] uppercase font-bold opacity-50 tracking-wider mb-2">
                    Status & Actions
                  </p>
                  <PaymentStatusSelect
                    orderId={order.id}
