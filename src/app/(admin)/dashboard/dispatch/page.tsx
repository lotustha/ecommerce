...
const MAX_CAPACITY_PER_RIDER = 10;
    ...
    return (
        <div className="space-y-6 pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* LEFT: Live Map Placeholder */}
                <div className="lg:col-span-2 bg-neutral rounded-3xl border border-neutral-focus shadow-lg overflow-hidden relative min-h-[400px] flex flex-col">
                    <div className="absolute top-4 left-4 z-10 bg-base-100/90 backdrop-blur-md px-4 py-2 rounded-xl shadow-lg border border-base-200 flex items-center gap-2 text-sm font-bold">
                        <Map size={16} className="text-primary" /> Live Tracking Map
                    </div>
                    <div className="absolute top-4 right-4 z-10 bg-base-100/90 backdrop-blur-md px-3 py-1.5 rounded-xl shadow-lg border border-base-200 flex items-center gap-2 text-xs font-bold text-success">
                        <Wifi size={14} /> WebSocket Ready
                    </div>
                    ...
                    <div className="absolute top-1/2 left-1/2 w-[800px] h-[800px] -translate-x-1/2 -translate-y-1/2 border border-primary/20 rounded-full animate-[spin_10s_linear_infinite] pointer-events-none">
                        <div className="w-1/2 h-1/2 border-r-2 border-primary opacity-50 rounded-tr-full"></div>
                    </div>

                    {/* Simulated Rider Pins (Will be replaced by real Google Maps Markers) */}
                    {fleetData.filter(r => r.activeCount > 0).map((rider, i) => (
                        <div
                            key={rider.id}
                        ...
                        >
                            {/* Tooltip */}
                            <div className="bg-base-100 text-base-content text-xs font-bold px-3 py-1.5 rounded-lg shadow-xl mb-1 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap border border-base-200">
                                {rider.name} • {rider.activeCount} parcels
                            </div>
                            {/* Pin */}
                            <div className="relative">
                                <span className="absolute -inset-2 bg-primary/30 rounded-full animate-ping"></span>
                                <div className="w-4 h-4 bg-primary border-2 border-base-100 rounded-full shadow-md relative z-10"></div>
                            </div>
                        </div>
                    ))}

                    <div className="mt-auto z-10 p-4 w-full bg-gradient-to-t from-neutral to-transparent text-neutral-content/60 text-xs text-center font-medium">
                        This interface is wired to receive live WebSocket connections from the Flutter Rider App.
                    </div>
                </div>
