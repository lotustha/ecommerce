...
import { ArrowLeft, User, MapPin, CreditCard, Package, Calendar, Mail, Phone, Truck, Wallet, ClipboardList } from "lucide-react";
...
import { getAvailablePartners } from "@/actions/delivery-actions";
        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/orders" className="btn btn-sm btn-ghost btn-circle">
                        <ArrowLeft size={20} />
                    </Link>
                    <div>
                        <h1 className="text-3xl font-black tracking-tight flex items-center gap-3">
                            Order #{order.id.slice(-6).toUpperCase()}
                            <span className={`badge font-bold ${order.status === 'DELIVERED' ? 'badge-success text-white' :
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-2 space-y-6">
                    ...
                    <div className="card bg-base-100 border border-base-200 shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-base-200 bg-base-200/30 flex justify-between items-center">
                            ...
                            <span className="text-xs font-bold opacity-60">{order.items.length} Items</span>
                        </div>
                                ...
                                <tbody>
                                    {order.items.map(item => (
                                        <tr key={item.id} className="hover:bg-base-200/30">
                                            ...
                                            <td className="text-right text-sm">{formatPrice(Number(item.price))}</td>
                                            <td className="text-center">{item.quantity}</td>
                                            <td className="pr-6 text-right font-bold">{formatPrice(Number(item.price) * item.quantity)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <div className="card bg-base-100 border border-base-200 shadow-sm">
                        <div className="card-body p-6">
                            <h3 className="font-bold flex items-center gap-2 mb-4 border-b pb-2 border-base-200">
                                <Wallet size={18} className="text-primary" /> Payment Information
                            </h3>
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    <p className="text-xs font-bold uppercase opacity-50 mb-1">Payment Method</p>
                                ...
                                <div>
                                    <p className="text-xs font-bold uppercase opacity-50 mb-1">Payment Status</p>
                                    <span className={`badge font-bold ${order.paymentStatus === 'PAID' ? 'badge-success text-white' : 'badge-warning text-warning-content'}`}>
                                        {order.paymentStatus}
                                    </span>
                                </div>
                            </div>
                ...
                <div className="space-y-6">
                    ...
                    {/* ✅ 1. CUSTOMER & ADDRESS (Top of sidebar) */}
                    <div className="card bg-base-100 border border-base-200 shadow-sm p-6 space-y-5">
                        <h3 className="font-bold flex items-center gap-2 border-b pb-3 border-base-200">
                            <ClipboardList size={18} className="text-primary" /> Customer Details
                        </h3>
