...
export default function OrderList({ orders = [] }: OrderListProps) {
  ...
  useEffect(() => {
    ...
    if (status === "success") {
      ...
      // Clean URL to prevent toast showing on refresh
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      default:
        return "text-base-content/50 bg-base-200";
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-base-content/60 mb-8 max-w-xs mx-auto">
          Looks like you haven't started your shopping journey yet.
        </p>
        <Link
          href="/search"
          className="btn btn-primary rounded-full px-8 shadow-lg hover:shadow-primary/20 hover:scale-105 transition-all"
        >
          Start Shopping
        </Link>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4">
      <header className="mb-10 flex items-end justify-between">
        ...
        <span className="text-sm font-medium bg-base-200 px-3 py-1 rounded-full">
          {orders.length} Total
        </span>
      </header>
      ...
      >
        {orders.map((order) => {
          ...
          // Logic: If status is UNPAID or FAILED, AND method is an online gateway (not COD)
          const showPayNow =
          ...
          return (
            ...
            >
              ...
              <div
                className={`absolute top-6 right-6 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5 ${getStatusColor(order.status)}`}
              >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4 pr-32">
                <div>
                  <p className="text-xs text-base-content/40 font-bold uppercase tracking-wider mb-1">
                  ...
                  <div className="flex gap-3 mt-1">
                    ...
                    <p
                      className={`text-sm flex items-center gap-1 font-bold ${order.paymentStatus === "PAID" ? "text-success" : "text-warning"}`}
                    >
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  {showPayNow && (
                    <Link
                      ...
                      className="btn btn-sm btn-error shadow-sm text-white flex-1 sm:flex-none"
                    >
                  ...
                  <Link
                    ...
                    className="btn btn-ghost btn-sm gap-1 group-hover:text-primary transition-colors flex-1 sm:flex-none"
                  >
                    Details{" "}
                    <ArrowRight
                      size={16}
