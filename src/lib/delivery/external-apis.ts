...
async function getPathaoConfig() {
  ...
  return {
    ...
    clientId:
      !isSandbox && settings?.pathaoClientId
        ? settings.pathaoClientId
        : process.env.PATHAO_CLIENT_ID || "QK9b69QaEv",
    clientSecret:
      !isSandbox && settings?.pathaoClientSecret
        ? settings.pathaoClientSecret
        : process.env.PATHAO_CLIENT_SECRET ||
          "k12nLGgq0zM3a65Sp65el4SZO6dhhMIxR0rDCavz",
    username:
      !isSandbox && settings?.pathaoUsername
        ? settings.pathaoUsername
        : process.env.PATHAO_USERNAME || "test.parcel@pathao.com",
    password:
      !isSandbox && settings?.pathaoPassword
...
export async function cancelPathaoOrder(consignmentId: string) {
  try {
    ...
    const res = await fetch(
      `${config.baseURL}/aladdin/api/v1/orders/${consignmentId}/cancel`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
