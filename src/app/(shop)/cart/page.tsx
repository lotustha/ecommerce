...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    const allItemKeys = new Set(
      items.map((item) => getItemKey(item.productId, item.variantId)),
    );
  ...
  // 1. Calculate Base Subtotal (Inclusive of Tax)
  const subTotal = selectedCartItems.reduce(
    (acc, item) => acc + item.price * item.quantity,
    0,
  );

  // 2. Dynamic Shipping & Free Shipping Logic
  ...
  // 3. Tax Calculation (INCLUSIVE Pricing)
  // If price is 113 and tax is 13%, Taxable Amount = 100, Tax = 13.
  ...
  // 4. Final Total (We DO NOT add taxAmount here, because it's already inside subTotal)
    ...
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-base-content/60 mb-8 max-w-md">
          Looks like you haven't added anything yet. Explore our categories to
              ...
              {subTotal >= freeThreshold ? (
                ...
                <div className="space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-sm font-medium">
                    ...
                    <span className="text-xs font-bold opacity-60 bg-base-100 px-2 py-1 rounded-md border border-base-200">
                      {Math.min(
                  ...
                  <progress
                    className="progress progress-primary w-full bg-base-300/50 h-3"
                    value={subTotal}
                    max={freeThreshold}
                  ></progress>
                </div>
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col justify-between py-0.5">
                    <div className="flex justify-between items-start gap-4">
                      ...
                      <button
                        onClick={() =>
                          handleRemove(item.productId, item.variantId ?? null)
                        }
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mt-3">
                      <div
                        className={`flex items-center bg-base-200 rounded-full h-9 px-1 border border-base-200 w-fit ${!isSelected ? "opacity-50 pointer-events-none" : ""}`}
                      >
                        <button
                          onClick={() =>
                            updateQuantity(
                      ...
                      <div className="text-left sm:text-right">
                        <div
                          className={`font-black text-lg ${isSelected ? "text-primary" : "text-base-content/50"}`}
                        >
          ...
          <div className="bg-base-100 border border-base-200 rounded-[2rem] p-8 shadow-xl relative overflow-hidden transition-all">
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex justify-between items-center text-base-content/70">
                ...
                <span
                  className={`font-bold ${shippingEstimate === 0 && selectedCartItems.length > 0 ? "text-success" : "text-base-content"}`}
                >
              ...
              {/* ✅ Tax Breakdown UI (Showing it's included) */}
              {taxRate > 0 && (
                <div className="flex justify-between items-center text-base-content/50 mt-4 pt-4 border-t border-base-200 border-dashed">
            ...
            <div className="mt-6 text-center">
              <Link
                href="/"
                className="inline-flex items-center gap-2 text-sm font-medium text-base-content/50 hover:text-primary transition-colors group"
              >
                <ArrowLeft
                  size={16}
