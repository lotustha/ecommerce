...
export default async function PaymentPage({ params }: PaymentPageProps) {
    ...
    // If already paid, redirect to orders history
    ...
    // ✅ Generate eSewa Config on the Server (Securely using crypto)
    // This prevents "Module not found: crypto" errors on the client
    let esewaConfig = null;
    if (order.paymentMethod === "ESEWA") {
        esewaConfig = getEsewaConfig(totalAmount, order.id);
    }
        ...
        <div className="max-w-xl mx-auto py-16 px-4">
            <div className="card bg-base-100 shadow-xl border border-base-200 text-center p-8">
                ...
                <p className="text-base-content/60 mb-8">
                    Order <strong>#{order.id.slice(-6).toUpperCase()}</strong> has been placed.
                    <br />
                ...
                <div className="bg-base-200/50 p-6 rounded-2xl text-left mb-8 space-y-3">
                    ...
                    <div className="flex justify-between">
                        ...
                        <span className="font-bold text-lg text-primary">Rs. {totalAmount.toLocaleString()}</span>
                    </div>
                </div>
                ...
                <PaymentButton
                    ...
                    esewaConfig={esewaConfig} // ✅ Pass server-generated config
                    customer={{
                ...
                <div className="mt-6 text-xs text-base-content/40">
                    Secure payment processing powered by {order.paymentMethod === 'ESEWA' ? 'eSewa' : 'Khalti'}.
                </div>
            </div>
        </div>
    );
